...
write.table(genotypes,file = "popu_smp.chromesome_t.txt",col.names = T,row.names = T,quote = F,sep = "\t")
# 创建SNP位置文件
snps_location <- data.frame(  
  snpid = rownames(genotypes),  
  chr   = geno_data[,1],  # CHROM column  
  pos   = as.numeric(geno_data[,2]),  # POS column
  stringsAsFactors = FALSE)
write.table(snps_location,file = "smps_pos.txt",col.names = F,row.names = F,quote = F,sep = "\t")
