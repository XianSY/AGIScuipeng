...
include { DESEQ2_QC as DESEQ2_QC_FEATURECOUNTS      } from '../modules/local/deseq2_qc'
...
workflow RNASEQ {
    ...
    ch_gtf_featurecounts = PREPARE_GENOME.out.gtf.map{
        it -> tuple(meta_total,it)
    }.groupTuple().view()
    ch_featurecounts_multiqc = Channel.empty()
    ch_genome_bam_total = BAM_MARKDUPLICATES_PICARD.out.bam.map{it->it[1]}.collect().view()
    if (!params.skip_alignment && !params.skip_qc && !params.skip_biotype_qc && biotype) {
        ...
        ch_deseq_featurecounts = FEATURECOUNTS.out.counts.map{
               it -> tuple(it[0],it[1],params.compare,params.group)
               }.groupTuple().view()
       
        DESEQ2_COUNTS(
            ch_deseq_featurecounts
        )
        ch_tpm_pca = FEATURECOUNTS.out.featCounts.map{it -> tuple(it[0],it[1],params.group)}.groupTuple().view()
 	TPM_PCA(
            ch_tpm_pca
        )
        ...
        MULTIQC_CUSTOM_BIOTYPE.out.counts.collect().map{
            it -> it[1]
        }.view()
    }
