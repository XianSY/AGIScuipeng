...
workflow PREPARE_GENOME {
    ...
    if( params.aligner =~ /bismark/ ){
        ...
        /*
         * Generate bismark index if not supplied  为参考基因组建立索引，如果有索引就不建立
         */
    ...
    // Aligner: bwameth
        ...
        /*
         * Generate bwameth index if not supplied 如果是用bwameth比对，则为它建立索引
         */
        ...
        if (params.fasta_index) {
            ...
            ch_kary = SAMTOOLS_FAIDX.out.kary  
      }

      /*
      * if annotation is GFF3,transfrom it to GTF
      */
        if (params.gff.endsWith("gff3") || params.gff.endsWith("gff")  ){
            GFF2GTF(
                params.gff
            )
           ch_annotation_file = GFF2GTF.out.gtf.map{
            row -> row[1]
           }
        }else{
           ch_annotation_file = Channel.value(file(params.gff))
        }
    ...
    samtools_kary = ch_kary                   // channel: path(genome.fasta)
    versions      = ch_versions.ifEmpty(null) // channel: [ versions.yml ]
    annotation_file = ch_annotation_file      // channel: path(genome_annotation)
}
